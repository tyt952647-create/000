    ...
    def detect_intersections(self, small_fishes):
        ...
        for i in range(len(small_fishes)):
            for j in range(i + 1, len(small_fishes)):
                ...
                if intersection:
                    self.intersection_points.append(intersection)
...
class AggressionController:
    """Controls firing aggression with 1000% max multiplier"""
...
# =========================
def main():
    print("🎮 HYBRID FISH DETECTION SYSTEM v5.0 ENHANCED")
    print("=" * 60)
    print("✨ Features:")
    ...
    print("➕ Press + to increase aggression (max 1000%)")
    ...
    try:
        while True:
            ...
            # ===========================
            # 🎯 INTELLIGENT FIRING WITH AGGRESSION CONTROL
            # ===========================
            ...
            # Target selection: small fish with high confidence
            optimal_targets = []
            ...
            stats = [
                ...
                f"📊 Aggression: {aggression_controller.aggression_level}% ({aggression_controller.aggression_level/100:.1f}x)",
            ...
            if key == ord('q'):
                ...
                print(f"📈 Aggression increased to {agg}% (Multiplier: {agg/100:.1f}x, Threshold: {thresh:.2f})")
            elif key == ord('-') or key == ord('_'):
                agg, thresh = aggression_controller.decrease()
                print(f"📉 Aggression decreased to {agg}% (Multiplier: {agg/100:.1f}x, Threshold: {thresh:.2f})")
