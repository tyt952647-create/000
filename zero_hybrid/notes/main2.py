...
def main():
    ...
    try:
        while True:
            ...
            # =========================
            # 🎯 AUTO FIRE (CLASS 0 CLUSTERS ONLY)
            # =========================
            cluster_targets = []

            for fish in fishes:
                if fish.class_id == 0 and fish.speed > 2:
                    cluster_targets.append(fish)

            # Only fire if cluster (2+ small fish)
            if len(cluster_targets) >= 2:
                cx = int(sum(f.x + f.w//2 for f in cluster_targets) / len(cluster_targets))
                cy = int(sum(f.y + f.h//2 for f in cluster_targets) / len(cluster_targets))
